...
Traefik provides declarative https routing for web facing services.
...
Addional stacks are provided as deployable services in the `stacks` folder.
The stacks will have this layout:
1. The path for stack is `stacks/{name_of_stack}`
2. The docker compose file is named `compose.yaml`
3. An `example.env` with stack specific environment variables.
